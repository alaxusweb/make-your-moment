...
**この手順を素材生成より先に完了させる。順序を逆にしない。**
manifest が35枠すべての題材とファイル名を先に確定させ、素材はその枠を埋めるために
生成する。画像を先に作って manifest を後から合わせると、次の破綻が起きる。

- 31日分の題材設計が事前に固まらず、重複や偏りが出る
- 35番がコンセプトシールだと知らずに描かれ、文字を置く余白がなくなる
- ファイル名と slot の対応が後付けになる
